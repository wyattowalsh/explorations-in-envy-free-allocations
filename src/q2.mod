...
param nonneg binary; # value determining if card(I) - card(P) is nonnegative
...
var y{P, I} binary; # indicator variable to determine sets after removing an item
...
subject to determineSetsMinusOneItem: # Ensure that size of new sets are one less than original
	sum {p in P, i in I} y[p,i] = nonneg * ((card(I))- (card(P)-1));

subject to setContinuity{p in P, i in I}:
	y[p,i] <= x[p,i];
	
subject to findWorstP {p1 in P, p2 in P, item in I: p1 <> p2}: # establishes the maximum value of envy across all people
